# test project<!-- omit in toc -->

![test project logo]

- [About test project](#about-test project)
 - [Demo](#demo)
...
<a name="about-test project"></a>
## 1. About test project

test project - is an open-source, self-hosted cryptocurrency payment processor. It uniquely serves as both a gateway and a merchant, enabling you to accept payments in various cryptocurrencies without fees and intermediaries.

<a name="demo"></a>
### 1.1. Demo

test project demo version is available from us (works in testnet network), so you can try it yourself without installing it:
...
**Details of the latest version can be found here:** https://github.com/vsys-host/test project/commits/main/

**Find the comprehensive list of API endpoints here https://test project/api/**

**Tutorial video:** https://www.youtube.com/watch?v=yYK_JAm1_hg

**Find the knowledge base here:** https://test project/kb/launch/what-is-test project

**Latest announcements:** https://test project/news

<a name="available-coins"></a>
## 2. Available coins
test project offers a direct way to receive BTC, ETH, LTC, DOGE, XMR, XRP, TRX, BNB, SOL, MATIC, AVAX, FIRO, USDT (ERC20, TRC20, BEP-20, Polygon, Avalanche), USDC (ERC20, TRC20, BEP-20, Polygon, Avalanche).
...
Create test project chart configuration file `values.yaml` with BTC, LTC, DOGE, XMR enabled:
...
# helm install -f values.yaml test project vsys-host/test project
```

Login to test project: http://\<ip>\:5000/
...
Create CRDs, replace "demo.test project" and "support@v-sys.org" with your own domain and email address:
...
kind: Certificate
metadata:
  name: test project-cert
  namespace: test project
spec:
  commonName: demo.test project
  secretName: test project-cert
  dnsNames:
    - demo.test project
  issuerRef:
...
After a few minutes, your test project should be reachable on https://\<your domain\> and have a valid SSL.
...
spec:
  valuesContent: |-
    additionalArguments:
      - "--certificatesresolvers.default.acme.email=acme@test project"
...
storageClassName: local-path
external_ip: 1.2.3.4
domain: my-test project.example.com
...
**Find the comprehensive list of endpoints here https://test project/api/**
### 5.1. Payment flow
The process of accepting payments through test project works as follows:
Once you have installed test project and see active Wallets in the test project admin panel, you can start configuring your store to work with test project. test project operates on an invoice-based system, where each created invoice in test project corresponds to a unique address for the respective cryptocurrency. This allows you to identify the customer who made the payment.

Creating an invoice requires passing the `X-test project-Api-Key` in the header, and you also need to know the cryptocurrency name to form the endpoint for the request.

To begin, you need to obtain a list of cryptocurrencies available for invoice creation from test project (these are the ones that are online and not disabled in the admin panel).
<p align="center">
...
Next, create an invoice for the selected cryptocurrency using the [endpoint](#invoice-creation) `/api/v1/{crypto_name}/payment_request`. An invoice in test project should be created at the stage when you already know the unique `order_id /invoice_id` in your system, and only for the cryptocurrency chosen by the customer (do not create invoices for all cryptocurrencies received from test project immediately, as this will generate excess addresses that will not be used later). Invoices in test project are unique, one invoice will be created in the test project system for one `external_id`. The customer can change their mind and generate an address for another cryptocurrency; in this case, simply create an invoice for the new cryptocurrency as usual, and test project will automatically update the information in the already created invoice and provide you with a new payment address. If the customer pays to the previously generated address, test project will process the payment, and you will receive a notification (callback) about this payment, as test project saves all addresses generated by the customer. For each cryptocurrency, the customer receives a unique cryptocurrency address linked to their specific `order_id` (invoice_id). This address remains the same even if you send another request to test project to create the same invoice.

When creating an invoice in test project, provide a `callback_url` to indicate where test project should send a notification (callback) upon receiving a transaction associated with this invoice. When transactions are received for the corresponding invoice, test project updates the status of the invoice. An invoice in test project can be in one of the following statuses: UNPAID, PARTIAL, PAID, OVERPAID. You receive the current status of the invoice in the callback.
...
- **PAID**: A fully paid invoice in test project. You can influence the thresholds for when an invoice is considered fully paid by using the appropriate setting in the wallet.
...
You can consider the test project invoice status when developing a payment module for your store, but be mindful when using the `overpaid_fiat`  value from the callback in the OVERPAID status.
By default, when adding a transaction and calculating the amount in fiat, test project uses the cryptocurrency exchange rate that was in effect at the time the invoice was created or updated. However, you can adjust this behavior by configuring the relevant field in the wallet settings.
<p align="center">
  <img src="https://github.com/user-attachments/assets/4ad2ee1f-652a-43da-81f3-8e913d931144" alt="image3">
</p>

If you have set “Recalculate invoice rate” to a value other than 0, then in the case of an invoice being paid after the specified time, the exchange rate will be the one at the moment the transaction is credited. When creating an invoice in test project, an object is returned that includes the `recalculate_after` field, allowing you to inform the customer of how long the current exchange rate will be held for them.

#### Static address mode (advanced)

For businesses that need a permanent deposit address per customer (e.g., exchanges, custodial flows, recurring deposits), test project supports a “static address” usage pattern **without** changing the core invoice mechanics:
...
2. **Keep the address static by reusing the same invoice.**
   Each invoice in test project maps to a unique blockchain address. By reusing that invoice, you keep the deposit address unchanged for the customer.
...
Go to test project and either view or generate a new API Key.
...
test project supports user authentication and authorization through an API Key generated by the user. Send the API Key as a header value to Authorization with the format: `X-test project-Api-Key {API Key}`
...
'https://demo.test project/api/v1/crypto'
```
**Successful response:**
```
{
...
- `amount`: The amount for which the invoice should be created in test project.
- `callback_url`: The URL to which test project will send notifications in the event of transactions related to the created invoices.

test project uses pair  `external_id` and `callback_url` as invoice identificator and update invoice in case repeating it on the next invoice creation requests.
**Example Curl request:**
```
curl --location --request POST 'https://demo.test project/api/v1/ETH/payment_request' \
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
...
**Example Curl request:**
```bash
curl --location --request POST 'https://demo.test project/api/v1/ETH/quote' \
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
--header 'Content-Type: application/json' \
...
**Example Curl request:**
```
curl --location --request GET 'https://demo.test project/api/v1/ETH-USDC/addresses' \
--header 'X-test project-Api-Key: nApijGv8djih7ozY'
```
**Successful Response:**
```
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/transactions/ETH/0xDCA83F12D963c7233E939a32e31aD758C7cCF307' \
--header 'X-test project-API-Key: nApijGv8djih7ozY'
```
**Successful Response:**
```
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/invoices/107' \
--header 'X-test project-API-Key: nApijGv8djih7ozY'
```
**Successful Response:**
```
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/tx-info/0xbcf68720db79454f40b2acf6bfb18897d497ab4d8bc9faf243c859d14d5d6b66/240' \
--header 'X-test project-API-Key: nApijGv8djih7ozY'
```
**Successful Response:**
```
...
**Curl Example:**
```
curl --location --request POST 'https://demo.test project/api/v1/decryption-key' \
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
...
**Endpoint:** `/metrics`
**Authorization:** HTTP Basic Auth using metric credentials. Metric credentials can be set by environment variables: `METRICS_USERNAME, METRICS_PASSWORD`. The default username/password is `test project/test project`.
**HTTP request method:**  GET
**Example Curl Request:**
```
curl --location --request GET 'https://demo.test project/metrics' \
...
test project allows you to make payouts through the API. The API calls related to payouts are somewhat different from those described earlier, as they use HTTP Basic Auth for authentication and authorization, utilizing your test project login and password.
...
- `callback_url`: (optional) The URL to which test project will send notifications when payouts occur.
...
**Curl Example:**
```
curl --location --request POST 'https://demo.test project/api/v1/ETH-USDC/payout' \
...
```
Note:
If enable_payout_callback is enabled in test project, a callback will be sent automatically upon a successful payout.
If a callback_url is provided in the request, test project will send the notification to that URL.
...
```
For sending XRP, you can optionally pass a `dest_tag`. If provided, the address should be given in the regular format, and test project will automatically convert it to X-address format. Alternatively, you can manually convert the XRP address to X-address format and pass it in the `dest` field; in this case, `dest_tag` does not need to be provided.
...
**Curl Example:**
```
curl --location --request POST 'https://demo.test project/api/v1/ETH-USDT/multipayout' \
--header 'Authorization: Basic  nApijGv8djih7ozY' \
--header 'Content-Type: application/json' \
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/ETH-USDC/task/7028c45b-0c88-483e-b703-dd455a361b2e' \
--header 'Authorization: Basic  nApijGv8djih7ozY' \
--header 'Content-Type: application/json'
```
**Successful Response:**
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/ETH/balance' \
--header 'X-test project-Api-Key: nApijGv8djih7ozY'
```
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/ETH-USDT/fee-deposit-address' \
--header 'X-test project-Api-Key: nApijGv8djih7ozY'
```
...
**Curl Example:**
```
curl --location --request GET 'https://demo.test project/api/v1/crypto/balances' \
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
--header 'Content-Type: application/json'
```
Example Request (subset of cryptos):
```
curl --location --request GET 'https://demo.test project/api/v1/crypto/balances?includes=BTC,ETH'
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
--header 'Content-Type: application/json'
```
Successful Response:
...
test project uses a short-lived in-memory TTL cache to speed up the /crypto/balances endpoints. Cached data about available cryptocurrencies and node status may be slightly outdated (up to 60 seconds). In multi-process deployments, each instance keeps its own cache, which may cause minor differences between responses. The cache reduces short-term node or network fluctuations but may briefly delay reflecting real-time status changes.
...
```
curl --location --request GET 'https://demo.test project/api/v1/BTC/payout/status?external_id=abc123' \
--header 'X-test project-API-Key: nApijGv8djih7ozY' \
--header 'Content-Type: application/json'
```
**Successful Response:**
...
The callback is sent to the specified `callback_url` provided during the invoice creation process. Typically, this is a script that receives the notification from test project, validates and processes it.

Once test project receives a payment, it will send a payment notification to the `<callback_url>` using a POST request with a JSON-encoded body and the `X-test project-Api-Key` HTTP header to authenticate with your system.

If a payment notification is successfully processed by your server, it should return the HTTP code `202 Accepted`. Any other response or connection failure will cause test project to resend the payment notification every 60 seconds.

test project will send a notification for each transaction related to the invoice, even if the invoice is already in the PAID/OVERPAID status. The transaction that triggered the callback is marked with the `trigger` field.
...
test project payment gateway module for WHMCS

Module has been tested on WHMCS Version: 8.10.1

Find module for WHMCS here: https://github.com/vsys-host/whmcs-test project-gateway-module

<a name="woocommerce-wordpress"></a>
#### 5.4.2. WooCommerce WordPress

test project payment gateway plugin for CMS WordPress + WooCommerce

Plugin has been tested on CMS WordPress 5.9.3 + WooCommerce 6.3.1

Find module for WooCommerce / WordPress here: https://github.com/vsys-host/wp-test project-plugin

<a name="opencart-3"></a>
#### 5.4.3. Opencart 3

test project payment gateway module for OpenCart 3

The module has been tested on CMS OpenCart Version 3.0.3.9

Find the module for Opencart 3 here: https://github.com/vsys-host/opencart-3-test project-payment-module

<a name="prestashop-8"></a>
#### 5.4.4. Prestashop 8

test project payment gateway module for Prestashop 8

The module has been tested on CMS Prestashop Version  8.1.7

Find the module for Prestashop 8 here: https://github.com/vsys-host/prestashop-8-test project-payment-module
...
test project features open-source code available from GitHub, which also means an excellent opportunity to be involved in the community. You can contribute to us, and if you do, we will appreciate it very much. After the validation, review, and test, we will publish your data. Should you consider this opportunity, please get in touch here.
...
If you have experienced any problems using test project, you can contact the community listed on the [official website](https://test project/). However, please ask questions on [Github Issues](https://github.com/vsys-host/test project/issues) related to technical issues only. Thank you.
...
[![Group 1790](https://github.com/user-attachments/assets/9829e615-2f22-4340-8d5d-eb5c156fcbcf)](https://www.reddit.com/user/test project_io/)
[![Group 1795](https://github.com/user-attachments/assets/24ee0fac-9f4d-4274-a482-18663f8a8f3f)](https://medium.com/@test project)
[![Group 1794](https://github.com/user-attachments/assets/e6803fff-7738-4148-a4d4-ed7d40faeeda)](https://x.com/test project_io)
[![Group 1793](https://github.com/user-attachments/assets/bba26cfd-fcdf-44ac-8046-6a03e06648b3)](https://www.facebook.com/test project)
...
Stay informed with the latest test project news, updates, and technical announcements. Follow our Telegram to never miss important changes and improvements:
https://t.me/test project_updates
